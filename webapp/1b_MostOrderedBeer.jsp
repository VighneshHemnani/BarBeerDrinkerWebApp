...
<% 
	...
	try{
		...
		Connection con = DriverManager.getConnection(url, "root", "Strongpassword123");			
   		...
   		//Make a query
   		String query = "" ;
   		query = "SELECT t1.item AS beer, SUM(t1.quantity) AS totalquantity FROM BarBeerDrinkerProject.transactions t1, BarBeerDrinkerProject.bills t2 WHERE t2.drinker = \'"+entity+"\' AND t2.bill_id = t1.bill_id AND t1.type = 'beer' GROUP BY beer ORDER BY totalquantity desc limit 10";
		...
		//Run the query against the database.
   		...
   		while (result.next()) { 
			map=new HashMap<String,Integer>();
		  	map.put(result.getString("beer"),result.getInt("totalquantity"));
			list.add(map);
   	    } 
   	    result.close();
        ...
        //Create the chart title according to what the user selected
     	chartTitle = "Top 10 beers ordered most by "+entity;
      	legend = "beers";
